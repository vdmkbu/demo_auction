...
@section('headline', 'Редактирование новости')
            ...
            <div class="ibox-content">
                <form enctype='multipart/form-data' method='post' action='{{ route('admin.news.update', $news->id) }}'>
                    ...
                    <div class="form-group row{{ $errors->has('name') ? ' has-error' : '' }}"><label class="col-sm-2 col-form-label">Заголовок</label>
                        <div class="col-sm-10">
                            <input type="text" name='name' required value="{{ old('name', $news->name) }}" class="form-control">
                            @if ($errors->has('name'))
                    ...
                    <div class="form-group row{{ $errors->has('date') ? ' has-error' : '' }}"><label class="col-sm-2 col-form-label">Дата</label>
                        <div class="col-sm-10">
                            <input type="text" name='date' value="{{ old('date', $news->date) }}" required class="form-control">
                            @if ($errors->has('date'))
                    ...
                    <div class="form-group row{{ $errors->has('announce') ? ' has-error' : '' }}"><label class="col-sm-2 col-form-label">Анонс новости</label>
                        <div class="col-sm-10">
                            <textarea name='announce' class="form-control">{{ old('announce', $news->announce) }}</textarea>
                    ...
                    <div class="form-group row{{ $errors->has('text') ? ' has-error' : '' }}"><label class="col-sm-2 col-form-label">Текст новости</label>
                        <div class="col-sm-10">
                            <textarea name='text' id="text" required class="form-control">{{ old('text', $news->text) }}</textarea>
                            @if ($errors->has('text'))
